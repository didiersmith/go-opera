package balancer_v2_vault

// abigen --pkg balancer_vault_v2 --abi=vault_ABI.json > vault.go
import (
	"math/big"
	"strings"
...
func GetPoolTokens(poolId [32]byte) []byte {
	data, err := sAbi.Pack("getPoolTokens", poolId)
	if err != nil {
		log.Error("Error packing getPoolTokens", "err", err)
	}
	return data
}

func UnpackGetPoolTokens(data []byte) struct {
	Tokens          []common.Address
	Balances        []*big.Int
	LastChangeBlock *big.Int
} {
	out, err := sAbi.Unpack("getPoolTokens", data)
	...
	outstruct.LastChangeBlock = out[2].(*big.Int)
	return *outstruct
}
